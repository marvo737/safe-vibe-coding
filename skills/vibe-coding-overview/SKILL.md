...
**重複論点の責務分界(複数領域に跨るとき):** APIキー管理は **security 主担当**(cost は副)、冪等性は **data design 主担当**(performance は副)、削除ポリシーは **data design 主担当**(legal は副)。主担当スキルで設計を確定し、副担当スキルで観点を補強する。
...
digraph flow {
  ...
  "設計+試算を先に提示" -> "合意後に実装";
  "設計+試算を先に提示" -> "Red Flag 検出?" [label="user push to skip"];
  "Red Flag 検出?" -> "該当領域を1-5から特定" [label="yes — 強制戻し"];
}
```
